...
| --- | --- | --- |
| `enabled` | `true` | Полностью отключить оглавление |
| `minHeadings` | `3` | Минимальное количество заголовков для показа. Значение `1` — показывать всегда |
| `position` | `auto` | Как оглавление ведёт себя на разной ширине: `auto` — колонка на широких экранах, сворачиваемый блок на узких; `aside` — только колонка, на узких экранах оглавления нет; `top` — только сворачиваемый блок на любой ширине |
| `collapsed` | `true` | Стартовое состояние сворачиваемого блока (закрыт). Блок построен на `<details>`, поэтому работа с клавиатуры и скринридерами достаётся бесплатно от браузера. Развёрнутое оглавление на телефоне — это экран прокрутки перед первым абзацем |
| `level` | `[2, 3]` | Какие заголовки включать. Принимает те же значения, что `outline` стандартной темы VitePress: число, пару `[min, max]` или `'deep'` (h2–h6) |
| `label` | из переводов | Заголовок оглавления |

Оглавление включено по умолчанию только на постах. Чтобы включить его на
отдельной странице `layout: page`:
