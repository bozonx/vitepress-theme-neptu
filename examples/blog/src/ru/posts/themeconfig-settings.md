...
Это поможет вам лучше разобраться как распределить параметры по уровню, если краткто то локазиованные строки и параметры для контретной локали располагаете в конфиге 3 уровня, а все остальное в конфиге 2 уровня
:::
...
`readingTime`, `links.*`). Полный справочник ключей `t` — в
[Переводах интерфейса](i18n-translations#ключи-t).
...
Оглавление строится из заголовков страницы автоматически.
...
Оглавление включено по умолчанию только на постах. Чтобы включить его на странице c `layout: page` нужно в ее frontmatter добавить toc: true:
...
::: tip
Чтобы выключить toc на остельном посте то добавиье `toc: false` в его frontmatter
:::
