...
- `enabled` — полностью отключить оглавление. По умолчанию `true`
- `minHeadings` — минимальное количество заголовков для показа. По умолчанию `3`. Значение `1` — показывать всегда
- `position` — управляет тем, как оглавление ведёт себя на разной ширине:
...
- `collapsed` — стартовое состояние сворачиваемого блока. По умолчанию `true` (закрыт). Сворачиваемый блок построен на `<details>`, поэтому работа с клавиатуры и скринридерами достаётся бесплатно от браузера. Развёрнутое оглавление на телефоне — это экран прокрутки перед первым абзацем
- `level` — какие заголовки включать. Принимает те же значения, что `outline` стандартной темы VitePress: число, пару `[min, max]` или `'deep'` (h2–h6)
- `layouts` — layouts, на которых показывать оглавление
- `label` — заголовок оглавления. Иначе берётся из переводов
