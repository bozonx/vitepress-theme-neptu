...
Запись с полем `header` вместо `text`/`href` рендерится как заголовок секции, а не как ссылка — удобно для разделения групп в `bottomLinks`.

`nav` также принимает массив в формате VitePress (`[{ text, link, activeMatch, items }]`) вместо объектного вида — для совместимости со стандартными конфигами.
...
Каждая иконка `socialLinks` поддерживает поля `title` (tooltip), `class`, `iconClass`
и флаги видимости `desktopOnly` / `mobileOnly` — они работают и в сайдбаре, и в футере сайта.
