---
title: Переводы интерфейса и корневая страница выбора языка
description: >
...
## Язык не из встроенного набора

Если нужного языка нет среди встроенных, создайте папку локали и задайте
минимум `lang`, `label` и ключи `t` в `_site.yaml`. Остальные строки интерфейса
унаследуются от английского через фолбэк, поэтому переводить можно постепенно —
начать с самых заметных (`search`, `tags`, `categories`, `readingTime`,
`links.*`) и дописывать остальные.
