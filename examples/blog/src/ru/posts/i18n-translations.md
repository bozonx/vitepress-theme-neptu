...
`extends` в `_site.yaml` наследует **весь конфиг** родительской локали — не только
переводы `t`, но и `lang`, `title`, `description` и весь `themeConfig`. Объекты
объединяются рекурсивно (deep-merge), цепочки поддерживаются на любую глубину,
циклы обнаруживаются автоматически. Подробнее — в
[Настройках themeConfig](themeconfig-settings#наследование-локалей-extends).
...
Создание папки `src/<locale>/` с файлом `_site.yaml` — это задача конфигурации
3-го уровня. Инструкция по добавлению региональных вариантов (`en-GB`, `es-MX`)
и языков не из встроенного набора (`uk`, `vi`) — в
[Настройках themeConfig](themeconfig-settings#добавление-новой-локали).
