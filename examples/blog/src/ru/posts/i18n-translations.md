...
переводами интерфейса. Встроенные локали `ar` и `he` уже содержат `dir: 'rtl'`,
поэтому направление текста переключается автоматически:

- инлайн-скрипт в `<head>` устанавливает `dir="rtl"` на `<html>` до первой
  отрисовки — это предотвращает мигание LTR→RTL;
- composable `useLocaleDir` обновляет `dir` при клиентских переходах между
  локалями;
- `LocaleSelector` и переключатель языка в навигации проставляют `dir` на
  ссылках.

Для пользовательских RTL-локалей (не из встроенного набора) задайте `dir: rtl`
в `_site.yaml` — тема подхватит это поле автоматически.

::: warning CSS-ограничение
Встроенные CSS-стили темы рассчитаны на LTR — только точечные
`/*rtl:ignore*/`-комментарии в scoped-стилях переключателя темы.
Для полноценного зеркального отображения сайдбара, навигации и других
элементов потребуется дополнительная стилизация.
:::
