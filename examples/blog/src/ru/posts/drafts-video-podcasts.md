...
Пункты меню идут ровно в том порядке, в котором вы их записали. Каждый пункт — `<id платформы>: <URL эпизода>`. Указывайте только те платформы, где эпизод реально опубликован.

Так же можете расширить список подкаст платформ, если вы не нашли нужную в стандартном списке, задав свои платформы в конфиге или прямо в список podcasts поста как в примере ниже.
...
`api.iconify.design` в браузере читателя. Если это нежелательно — используйте `iconUrl` со своим
...
> Рекомендуется здесь размещать ссылку на пост в социальных сетях где опубликован пост связанный с данным постом для того чтобы направлять пользователь на ваши соц сети, на которые он могут подписаться.

---

Далее [Домашняя страница](home-page)
