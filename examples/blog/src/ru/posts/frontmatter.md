...
# Можно указывать как в виде даты в формате `YYYY-MM-DD`, так и со временем в формате ISO 8601 `YYYY-MM-DDTHH:mm:ss`, например `2026-07-29T12:00:00Z`, где `Z` означает UTC.
# Время имеет смысл указывать если в один день вы публикуете несколько постов, для того чтобы сохранить их порядок в списках
...
# Это поле просто синтаксический сахар, используется в случае когда нужно задать только одну категорию, но фактически значение переходит в поле `categories` как единственный элемент массива
category: writing
