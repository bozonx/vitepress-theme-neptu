...
Файл — массив профилей. Обязателен только `id`, остальные поля опциональны.
...
- **`avatar`** *(строка)* — URL компактного аватара для превью в списках. В текущей версии темы компоненты используют только `image`, поле предусмотрено в схеме на будущее.
- **`image`** *(строка)* — URL портрета автора. Используется на странице автора и в карточке автора в футере поста.
- **`imageWidth`** *(число)* — ширина `image` в пикселях для атрибута `width` тега `<img>`. Рекомендуется для предотвращения сдвига макета (CLS). Определяется автоматически по URL, но можно задать вручную.
