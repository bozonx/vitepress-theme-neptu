...
Смыл разделения на уровни в том чтобы, во первых, отделить конфиг связанный с системными настройками, которые настраивает разработчик от конфига который настраивает администратор блога, а во вторых разделить конфиг на глобальный и локализованный, чтобы не дублировать одни и теже параметры для каждой локали.

Поэтому конфиги 2го и 3го уровня, предназначенные для администратора, используеют формат YAML, который легко редактировать даже не программисту через какую-нибудь CMS, например Sveltia CMS, Decap CMS и подобные.

Администратор может, не залазя в системные настройки, отредактировать глобальный конфиг `src/site.yaml` через CMS что повлияет на все локали и так же может скорректировать парамтеры для конкретной локали в `src/<локаль>/_site.yaml`.
