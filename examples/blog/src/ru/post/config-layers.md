...
> Таким образом, `${theme.*}`, `${config.*}`, `${t.*}` и `${localeIndex}` разрешаются уже в первом проходе, а `${site.*}` — во втором. Циклические ссылки безопасны: итеративный проход ограничен 8 шагами, неразрешённые шаблоны остаются в тексте.
