...
> Разверни мой VitePress-блог на Cloudflare Pages. Создай `wrangler.toml` или
> настрой проект через Cloudflare Dashboard. Команда сборки — `npm run build`,
> каталог публикации — `src/.vitepress/dist`. Node.js 22+.

**Netlify:**

> Настрой деплой блога на Netlify. Создай `netlify.toml` с командой сборки
> `npm run build` и каталогом публикации `src/.vitepress/dist`.

**Vercel:**

> Разверни блог на Vercel. Создай `vercel.json` с настройками для статического
> сайта: build-команда `npm run build`, output-директория `src/.vitepress/dist`.

**GitHub Pages (автоматизация):**

> Создай GitHub Actions workflow для автоматического деплоя блога на GitHub Pages
> при пуше в `main`. База — `/<repo>/`, сборка через `npm run build`,
> артефакт из `src/.vitepress/dist`.
