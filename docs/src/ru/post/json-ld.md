...
Поле frontmatter `jsonLd` — это YAML, который **объединяется (merge)** со
сгенерированной схемой — вы указываете только то, что отличается или что тема
не может определить сама. В этом посте тип изменён на `TechArticle` и добавлены
