...
Подходящая ссылка получает пометку «Рекомендуется», но сайт не переходит по ней
автоматически. Пользователь всегда сам выбирает язык. Это также предотвращает
нежелательные переходы для многоязычных пользователей и поисковых роботов.
