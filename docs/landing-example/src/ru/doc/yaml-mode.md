...
Неизвестный `type` пропускается, а в dev-режиме в консоль выводится
предупреждение со списком известных типов — опечатка видна сразу, а не
превращается в молча пропавшую секцию.
