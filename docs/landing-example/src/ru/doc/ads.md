...
Правая колонка лендинга — это колонка оглавления **стандартной темы VitePress**:
оглавление настраивается через `themeConfig.outline`, а не через `toc`, и
показывается с 1280px. Тема пробрасывает все слоты стандартной темы, так что
любой блок можно поставить руками.

Рекламная часть при этом общая с блог-темой: те же `themeConfig.ads` и тот же
компонент `<NeptuAd />`.

::: tip Что отличается от блога
В блог-теме правая колонка своя: фиксированные 300px, показ от 1550px, в ней же
живёт оглавление темы (`themeConfig.toc`). Здесь колонка принадлежит стандартной
теме VitePress. Настройки `ads` и `consent` — одни и те же.
:::

## Быстрый путь: `ads.component`

Проще всего не трогать слоты вообще: зарегистрируйте свой компонент глобально и
назовите его в конфиге.
...
themeConfig: {
  ads: {
    component: 'AdUnit',
    layouts: ['doc'], // по умолчанию: только страницы документации
    aside: true,      // блок в правой колонке, под оглавлением
    inContent: { enabled: true }, // блоки внутри текста
  },
}
```

Тема сама поставит блок в слот `aside-ads-before` — под оглавлением, чтобы
оглавление осталось наверху. Блоки внутри текста подставляются **на этапе
сборки** плагином markdown-it: без скачка вёрстки и без правки DOM после
загрузки. Полное описание параметров — в статье блога «Оглавление, правая
колонка и реклама».

Если передать слот `aside-ads-before` самостоятельно, тема свой блок рисовать не
будет — ваш вариант выигрывает.

::: warning Согласие на куки
Для показа рекламы Google пользователям из ЕЭЗ и Великобритании нужна
сертифицированная CMP с поддержкой IAB TCF v2.2 — самописный баннер не подходит.
Тема выставляет значения Google Consent Mode v2 в `<head>` и даёт
`useConsent()`, но баннер не рисует. Подробности — в статье блога «Согласие на
куки и аналитика».
:::
