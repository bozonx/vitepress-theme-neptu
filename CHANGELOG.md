...
- `packages/blog`, `packages/landing` **breaking**: collapsed the canonical-link settings. `themeConfig.seo.autoCanonical` is gone — `themeConfig.seo.canonical` now means "emit a self-canonical automatically", and pages with an explicit `canonical` frontmatter value still get their link when it is `false`. `seo.rssLinks` is gone too — the feed `<link>` tags are controlled by `seo.rss` alone. Every `seo.*` key now resolves through one shared layer order: page frontmatter → locale `_site.yaml` → site `site.yaml` (previously the locale layer applied to `autoCanonical` only). Remove `autoCanonical` and `rssLinks` from your configs and frontmatter.
